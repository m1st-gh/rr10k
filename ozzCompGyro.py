...
with open('ANGLE_DATA.csv', 'w') as angle_, open('COMPASS_DATA.csv','w') as compass_, open('ACCEL_DATA.csv','w') as accel_:
    angle_writer = csv.writer(angle_)
    compass_writer = csv.writer(compass_)
    accel_writer = csv.writer(accel_)
    angle_writer.writerow(['X','Y', 'TIME (S)'])
    accel_writer.writerow(['X','Y','Z','TIME (S)'])
    timecur = 0.0
